...
$total_records = $conn->select_fetch('users', ['COUNT(*) as count'], ['role'], [0]);
...
$data_arr = $conn->select_join('users', 'students', ['users.*', 'students.*', 'users.id as user_id'], 'users.id = students.user_id WHERE users.role = 0 ' . $limit);
